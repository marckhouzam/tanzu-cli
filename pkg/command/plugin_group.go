...
func displayGroupContentAsTable(group *plugininventory.PluginGroup, specifiedVersion string, writer io.Writer) {
	...
	outputStandalone := component.NewOutputWriter(writer, outputFormat, "Name", "Target", "Version")
	...
	if showNonMandatory {
		fmt.Println("")
		log.Infof("The standalone plugins in this plugin group are installed when the 'tanzu plugin install --group %s%s' command is invoked.", groupID, specifiedVersion)

		fmt.Println("")
		outputContext := component.NewOutputWriter(writer, outputFormat, "Name", "Target", "Version")
...
func displayGroupContentAsList(group *plugininventory.PluginGroup, writer io.Writer) {
	output := component.NewOutputWriter(writer, outputFormat, "Group", "PluginName", "PluginTarget", "PluginVersion", "Context-Scoped")
